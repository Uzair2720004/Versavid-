...
import { Sparkles, ArrowRight, Mail, Lock, Globe2 } from 'lucide-react';
        ...
        <div className="relative z-10 flex flex-col justify-end p-12">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1], delay: 0.3 }}>
            <div className="glass rounded-full px-4 py-1.5 mb-5 inline-flex items-center gap-2 text-[12px] text-white/70">
              <Sparkles className="h-3.5 w-3.5 text-cyan-400" /> AI-powered YouTube automation
            </div>
            <h2 className="text-[40px] font-bold leading-[1.05] tracking-tightest text-white max-w-sm">
              Turn any topic into a YouTube video.
            </h2>
            <p className="mt-4 text-[14px] text-[#a8aeb8] max-w-xs leading-[1.5]">
              Script, visuals, voiceover, captions — all generated automatically by Versavid.
            </p>
            <div className="mt-8 flex items-center gap-4">
              {[{ n: '10K+', l: 'Videos generated' }, { n: '15', l: 'Free credits' }, { n: '8-step', l: 'AI pipeline' }].map((s) => (
                <div key={s.n} className="glass rounded-xl px-4 py-3">
                  <p className="text-[18px] font-bold text-white">{s.n}</p>
                  <p className="text-[10px] text-[#767D88] mt-0.5">{s.l}</p>
                </div>
              ))}
            </div>
          </motion.div>
      ...
      <div className="w-full lg:w-[480px] shrink-0 flex flex-col items-center justify-center px-8 py-12 relative">
        ...
        <div className="relative z-10 w-full max-w-sm">
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, ease: [0.16, 1, 0.3, 1], delay: 0.1 }}>
            <h1 className="text-[28px] font-bold tracking-tight text-white mb-1">Welcome back</h1>
            <p className="text-[13px] text-[#767D88] mb-8">Sign in to your Versavid studio</p>
            ...
            <p className="mt-4 text-center text-[11px] text-[#767D88]">
              <a href="#" className="hover:text-white transition-colors">Forgot password?</a>
            </p>
            <p className="mt-6 text-center text-[12px] text-[#767D88]">
              Don&apos;t have an account?{' '}
              <Link href="/signup" className="text-cyan-400 hover:text-cyan-300 transition-colors font-medium">Sign up free</Link>
            </p>
          </motion.div>
